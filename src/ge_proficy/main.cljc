(ns ge-proficy.main
  "Kotodama WASM entrypoint for the Ge_proficy clean-room actor (L4) — Clojure port.
  ...
  facts (`ge_proficy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ge_proficy")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"         :plural "tags"         :id-prefix "geprofic_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "geprofic_con"
    :fields [:name :setpoint :processValue :mode]     :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float}   :refs {}}
   {:entity "Batch"       :plural "batches"      :id-prefix "geprofic_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"ge_proficy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ge_proficy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ge_proficy-compat/ready)
